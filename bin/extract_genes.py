...
logFormatter = logging.Formatter(
    '%(asctime)s %(levelname)-8s [extract_genes.py] %(message)s'
)
...
if aln.shape[0] > 0:
    ...
    if aln.shape[0] > 0:
        ...
        # Make a dict with the aligned sequence for each gene
        gene_sequences = dict()

        # Iterate over each gene
        for gene_name, gene_aln in aln.groupby("sseqid"):

            # Get the aligned sequence for the top-scoring alignment
            gene_sequences[
                gene_name
            ] = get_aligned_region(
                gene_aln.sort_values(by="score", ascending=False).head(1).iloc[0],
                genome
            )

        # Write out to a file
        with gzip.open(f"{genome_fp}.fasta.gz", "wt") as handle:
            for gene_name, gene_sequence in gene_sequences.items():
                handle.write(f">{gene_name}::{genome_name}\n{gene_sequence}\n")
