...
def main(read_alignments: pd.DataFrame, gene_bins: pd.DataFrame):
    ...
    """
    logger.info("Calculating bin size")
    bin_size = gene_bins["bin"].value_counts().to_dict()
    ...
    (
        ...
        .apply(lambda d: summarize_bin(d, bin_size))
        .to_csv("bin_summary.csv.gz")
    )


def summarize_bin(df: pd.DataFrame, bin_size: dict) -> pd.Series:
    """
    ...
    _bin_size = bin_size[df["bin"].iloc[0]]
    prop_genes_detected = n_genes_detected / _bin_size
    # Coverage of genes in this bin
    bin_coverage = df["coverage"].mean() * prop_genes_detected
